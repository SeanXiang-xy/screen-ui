...
export const addVersion = async () => {
  await run("pnpm version patch", rootPath);
};
// dist 添加package.json
export const setPackageJson = async () => {
  const oriData = require(`${rootPath}/package.json`);
  const pkgInfo = require("./pkg/package.json");
  pkgInfo.dependencies = oriData.dependencies;
  pkgInfo.version = oriData.version;
